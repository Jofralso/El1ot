...
Professional cybersecurity dashboard with progressive disclosure,
dark-first design, and proper navigation structure.
"""
...
/* ── Topology SVG ── */
.topo-container { width:100%; height:400px; background:var(--bg-root); border:1px solid var(--border); border-radius:8px; overflow:hidden; position:relative; }
...
<div class="app">
  ...
  <div class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <div class="nav-item" data-page="agent" onclick="showPage('agent')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20z"/><path d="M12 16v-4M12 8h.01"/></svg>
        Agent
...
function showPage(page) {
  ...
  const titles = {
    ...
    findings: 'Findings', intel: 'Intel', knowledge: 'Knowledge Base',
    agent: 'Agent', settings: 'Settings'
  };
  document.getElementById('page-title').innerHTML = titles[page] || page;
  const renderers = { overview:renderOverview, map:renderMap, scan:renderScan, findings:renderFindings, intel:renderIntel, knowledge:renderKnowledge, agent:renderAgent, settings:renderSettings };
...
async function renderMap() {
  const c = document.getElementById('content');
  c.innerHTML = '<div class="fade-in"><div class="topo-container" id="topo"></div><div style="margin-top:16px" id="map-table"></div></div>';
  ...
  const devs = devices.devices || [];
  let rows = devs.map(d => `<tr><td><code style="color:var(--accent)">${d.ip}</code></td><td>${d.hostname||'-'}</td><td>${d.mac||'-'}</td><td>${d.os_guess||'-'}</td><td><span class="sev sev-${d.type==='router'?'high':'info'}">${d.type||'?'}</span></td><td>${(d.services||[]).length}</td></tr>`).join('');
...
function renderTopologySVG(topo) {
  ...
  topo.nodes.forEach(n => {
    ...
    svg += `<g class="topo-node" transform="translate(${p.x},${p.y})">`;
...
async function loadSuggestions() {
  ...
  try {
    ...
    const data = await res.json();
    el.innerHTML = `<div class="suggestions">${(data.suggestions||[]).map(s=>`<div class="suggestion" onclick="document.getElementById('chat-input').value='${s.replace(/'/g,"\\'")}';sendChat()">${s}</div>`).join('')}</div>`;
...
async function renderKnowledge() {
  ...
  const [knowledge, tamagotchi] = await Promise.all([
    fetch(API+'/tamagotchi/knowledge').then(r=>r.json()).catch(()=>({entries:[],stats:{}})),
    fetch(API+'/tamagotchi/knowledge').then(r=>r.json()).catch(()=>({stats:{}}))
  ]);

  const stats = knowledge.stats||{};
  const entries = knowledge.entries||[];

  const rows = entries.slice(0,50).map(e => `<tr>
    <td><span class="sev sev-${e.severity||'info'}">${(e.severity||'info').toUpperCase()}</span></td>
    <td style="font-weight:500;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${e.title||e.category||'-'}</td>
    <td style="font-size:12px;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${e.content||e.summary||'-'}</td>
  ...
  document.getElementById('knowledge-content').innerHTML = `
    <div class="stats-grid" style="grid-template-columns:repeat(4,1fr)">
      <div class="stat-card"><div class="stat-label">Total Entries</div><div class="stat-value accent">${stats.total||0}</div></div>
      <div class="stat-card"><div class="stat-label">Vulnerabilities</div><div class="stat-value critical">${stats.vulnerabilities||0}</div></div>
      <div class="stat-card"><div class="stat-label">Techniques</div><div class="stat-value medium">${stats.techniques||0}</div></div>
      <div class="stat-card"><div class="stat-label">Networks</div><div class="stat-value success">${stats.networks||0}</div></div>
    </div>
    <div class="card">
      <div class="card-header">Knowledge Base</div>
      <div class="card-body table-wrap"><table><thead><tr><th>Sev</th><th>Title</th><th>Content</th><th>Date</th></tr></thead><tbody>${rows||'<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:30px">Knowledge base empty. Using LLM during chat builds the index.</td></tr>'}</tbody></table></div>
    </div>`;
}
...
async function renderAgent() {
  const c = document.getElementById('content');
  c.innerHTML = '<div class="fade-in" id="agent-content"></div>';

  const [tamagotchi, notifs, cracks] = await Promise.all([
    fetch(API+'/tamagotchi/status').then(r=>r.json()).catch(()=>({})),
    fetch(API+'/tamagotchi/notifications').then(r=>r.json()).catch(()=>({notifications:[]})),
    fetch(API+'/tamagotchi/cracks').then(r=>r.json()).catch(()=>({sessions:[]}))
  ]);
  ...
  const notifRows = (notifs.notifications||[]).slice(0,20).map(n => `<tr>
    <td><span class="sev sev-${n.severity||'info'}">${(n.severity||'info').toUpperCase()}</span></td>
    <td style="font-weight:500">${n.title}</td>
    <td style="font-size:12px;max-width:250px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${n.message}</td>
    <td style="font-size:12px;color:var(--text-muted)">${n.target||'-'}</td>
    <td><span class="status status-${n.auth_status==='approved'?'fixed':n.auth_status==='denied'?'accepted':n.auth_status==='auto_approved'?'progress':'open'}">${n.auth_status||'info'}</span></td>
    <td>${n.auth_status==='pending'?`<button class="btn btn-sm btn-primary" onclick="authNotif('${n.id}')">Approve</button> <button class="btn btn-sm" onclick="denyNotif('${n.id}')">Deny</button>`:''}</td>
  </tr>`).join('');

  const crackRows = (cracks.sessions||[]).map(s => `<tr>
    <td>${s.tool}</td>
    <td style="font-size:12px;font-family:'JetBrains Mono',monospace">${s.hash_file}</td>
    <td><span class="status status-${s.status==='completed'?'fixed':s.status==='running'?'progress':'open'}">${s.status}</span></td>
    <td style="font-size:11px;color:var(--text-muted)">${s.started_at?new Date(s.started_at*1000).toLocaleTimeString():'-'}</td>
  </tr>`).join('');

  document.getElementById('agent-content').innerHTML = `
    <div class="stats-grid" style="grid-template-columns:repeat(4,1fr)">
      <div class="stat-card"><div class="stat-label">State</div><div class="stat-value" style="font-size:20px;color:${stateColors[tamagotchi.state]||'var(--text-muted)'}">${tamagotchi.state||'idle'}</div></div>
      <div class="stat-card"><div class="stat-label">Notifications</div><div class="stat-value accent">${(notifs.notifications||[]).length}</div></div>
      <div class="stat-card"><div class="stat-label">Exploits Queued</div><div class="stat-value medium">${(tamagotchi.exploit_queue||[]).length}</div></div>
      <div class="stat-card"><div class="stat-label">Crack Sessions</div><div class="stat-value warning">${(cracks.sessions||[]).length}</div></div>
    </div>
    <div class="card" style="margin-bottom:16px">
      <div class="card-header">Notifications & Authorization</div>
      <div class="card-body table-wrap"><table><thead><tr><th>Sev</th><th>Title</th><th>Message</th><th>Target</th><th>Status</th><th>Action</th></tr></thead><tbody>${notifRows||'<tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:30px">No notifications</td></tr>'}</tbody></table></div>
    </div>
    <div class="card">
      <div class="card-header">Cracking Sessions</div>
      <div class="card-body table-wrap"><table><thead><tr><th>Tool</th><th>Target</th><th>Status</th><th>Started</th></tr></thead><tbody>${crackRows||'<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:30px">No active sessions</td></tr>'}</tbody></table></div>
    </div>`;
}

async function authNotif(id) {
  await fetch(API+'/tamagotchi/authorize', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({notification_id:id})});
  renderAgent();
}
async function denyNotif(id) {
  await fetch(API+'/tamagotchi/deny', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({notification_id:id})});
  renderAgent();
}
...
connectWS();
showPage('overview');
setInterval(()=>{ if(currentPage==='overview') renderOverview(); }, 30000);
</script>
</body>
</html>"""
